...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        let newArr = arr.filter((task: any) => (new Date(task.dueDate)).toISOString().split('T')[0] === (new Date()).toISOString().split('T')[0])
    ...
    const submitHandler = () => {
        ...
        if (title) {
            const alreadyStoredData = localStorage.getItem('todaytasks')

            if (alreadyStoredData) {
                let tasksArray = JSON.parse(alreadyStoredData);
                const newArr = [...tasksArray, { ...data, id: Math.random() }]
                localStorage.setItem('todaytasks', JSON.stringify(newArr))
                setAllTasks(newArr)
            } else {
                localStorage.setItem('todaytasks', JSON.stringify([{ ...data, id: Math.random() }]))
    ...
    const updateHandler = () => {
        ...
        if (title) {
            allTasks[id] = { ...data }
            localStorage.setItem("todaytasks", JSON.stringify([...allTasks]));
